...
const Page = ({ params: asyncParams }) => {
  ...
  //   queryKey: ["refresh"],
  //   queryFn: () => getCarRefresh("refresh"),
  // });

  useEffect(() => {
    // if (refreshResponse?.data?.packageDetails?.length) {
    //   const match = refreshResponse.data.packageDetails.find(
    //     (item) => String(item.id) === String(id)
    //   );
    //   if (match) {
    //     setSelectedCompany({
    //       companyName: match.companyName,
    //       numberOfRefreshes: match.totalQuantity - match.consumedQuantity,
    //     });
    //   }
    // }
  }, [selectedRow, id]);
  ...
  const schema = yup.object({
    numberOfRefreshes: yup
      .number()
      .typeError("Number must be a number")
      .min(1, "At least 1 refresh required")
      .required("Refresh count is required"),
  });
  ...
  useEffect(() => {
    if (selectedRow) {
      ...
      setValue("numberOfRefreshes", selectedRow.total_quantity);
    }
  ...
  const fields = [
    ...
    {
      type: "input",
      name: "numberOfRefreshes",
      placeholder: "Enter the Number",
      label: "No of Refreshes",
      inputtype: "number",
      req: true,
      colWidth: "col_md_6",
    },
  ...
  const onSubmit = (data) => {
    const quantity = parseInt(data.numberOfRefreshes, 10);
