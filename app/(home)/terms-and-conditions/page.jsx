import React from 'react'

function page() {
    return (
        <section className="termsSec">
            <div className="customContainer">
                <div className="headingCont desc">
                    <h1>
                        Terms & Conditions
                    </h1>
                    <p>
                        Welcome to OneTapDrive! By using our website or app, you have to fully agree to the given terms
                        and conditions that are here to ensure your experience is smooth and hassle-free.
                    </p>
                </div>
                <div className="content">
                    <h3>
                        What Should You Know
                    </h3>
                    <ul>
                        <li>
                            <span>
                                How Our Service Works
                            </span>
                            <p>
                                OneTapDrive simplifies the process of car rental. Just visit our website or app to
                                reserve a car, and you'll have access to a wide range of options for daily, weekly, or
                                monthly rentals.
                            </p>
                        </li>
                        <li>
                            <span>
                                Agreeing to the Terms
                            </span>
                            <p>
                                While making a booking through our website, you are agreeing to all the terms and
                                conditions. If something doesn't feel right, you can always choose another service that
                                works better for you.
                            </p>
                        </li>
                        <li>
                            <span>
                                What We Offer
                            </span>
                            <p>
                                We help you connect with trusted suppliers so you can easily find the right car. Whether
                                you're looking for cheap cars or a larger SUV, we make the booking process simple, but
                                the supplier handles the actual rental and payment directly.
                            </p>
                        </li>
                    </ul>
                    <h3>
                        How to Book
                    </h3>
                    <p>
                        Complete your reservation on our platform, and you'll get a final, upfront price—no surprises or
                        hidden fees. Once confirmed, you'll receive an invoice at the email you provide.
                    </p>
                    <h3>
                        Payments
                    </h3>
                    <p>
                        Payments are straightforward: use your credit card. However, any additional fees, like currency
                        conversion charges, are your responsibility. The amount displayed during booking is exactly what
                        you'll be charged.
                    </p>
                    <h3>
                        Keeping Things Smooth
                    </h3>
                    <ul>
                        <li>
                            <span>
                                Platform Changes
                            </span>
                            <p>
                                We're always improving. Sometimes, updates or maintenance may result in temporary
                                changes or pauses in service, but these are designed to make your experience better.
                            </p>
                        </li>
                        <li>
                            <span>
                                Exclusive Promotions
                            </span>
                            <p>
                                We often roll out special deals and discounts. These offers are subject to availability
                                and may vary depending on the car type, location, or rental period. Be sure to check in
                                regularly to catch the latest promotions.
                            </p>
                        </li>
                        <li>
                            <span>
                                Your Responsibility
                            </span>
                            <p>
                                It's important to double-check the information you provide during booking. Accurate
                                details ensure a seamless rental experience.
                            </p>
                        </li>
                        <li>
                            <span>
                                Privacy and Your Data
                            </span>
                            <p>
                                Your personal information is handled with care. Please review our Privacy Policy to
                                understand how your data is managed.
                            </p>
                        </li>
                        <li>
                            <span>
                                Our Role as a Facilitator
                            </span>
                            <p>
                                OneTapDrive acts as an intermediary, connecting you with car suppliers. We don't provide
                                the vehicles ourselves. All transactions and claims are handled directly with the
                                supplier.
                            </p>
                        </li>
                        <li>
                            <span>
                                After Booking
                            </span>
                            <p>
                                Once your booking is confirmed and payment is processed, we'll send a confirmation email
                                with your invoice. When driver details are shared, you can directly coordinate with the
                                provider for your rental.
                            </p>
                        </li>
                    </ul>
                    <h3>
                        Behavior Guidelines
                    </h3>
                    <p>
                        Local laws require all passengers to wear seatbelt(s). Please follow the driver's instructions
                        during your trip. Prohibited actions include opening doors while the car is moving, throwing
                        objects outside, or leaning out of the windows. Smoking and drinking alcohol in the vehicle are
                        strictly forbidden. Violations may result in cleaning charges or compensation for downtime.
                    </p>
                    <h3>
                        Unforeseen Delays
                    </h3>
                    <p>
                        Occasional delays or cancellations caused by events like strikes or extreme weather are part of
                        the service limitations. Compensation in such cases may be limited.
                    </p>
                    <h3>
                        Vehicle Capacity and Safety
                    </h3>
                    <p>
                        Passengers and luggage must adhere to the vehicle's safety limits. If there's a safety concern,
                        the driver may refuse excess passengers or luggage.
                    </p>
                    <h3>
                        Vehicle Options
                    </h3>
                    <p>
                        Our fleet includes various categories, like Luxury and Standard. Images shown during booking are
                        for reference, and the actual car may differ. If the reserved vehicle isn't available, we'll
                        provide an upgraded option whenever possible.
                    </p>
                    <h3>
                        Using Our Content
                    </h3>
                    <p>
                        We grant you a temporary license to view and download content from OneTapDrive's site for
                        personal, non-commercial purposes. However, this license does not transfer ownership.
                    </p>
                    <h3>
                        Here's what you cannot do:
                    </h3>
                    <ul>
                        <li>
                            <p>
                                Alter or reproduce our content.
                            </p>
                        </li>
                        <li>
                            <p>
                                Use it for public displays or commercial purposes.
                            </p>
                        </li>
                        <li>
                            <p>
                                Attempt to dismantle or reverse-engineer our software.
                            </p>
                        </li>
                        <li>
                            <p>
                                Remove copyright or ownership notices.
                            </p>
                        </li>
                        <li>
                            <p>
                                Share or repost our content elsewhere.
                            </p>
                        </li>
                    </ul>
                    <p>
                        If you breach any of these conditions, your license will automatically terminate. OneTapDrive
                        reserves the right to revoke access at any time. Upon termination, you must delete any
                        downloaded materials, whether digital or printed.
                    </p>
                    <h3>
                        Your Responsibilities, Refunds, and Cancellations
                    </h3>
                    <ul>
                        <li>
                            <p>
                                At OneTapDrive, renting a car becomes a seamless process through the website or app.
                                Understanding how everything works is essential. An account is created by signing up
                                with a valid email and password. Connections are made to third-party service providers,
                                allowing you to browse, book, and pay while they handle the services directly.
                            </p>
                        </li>
                        <li>
                            <p>
                                The role of OneTapDrive is clearly outlined. Cars, pricing, and details shown on the
                                platform are provided by suppliers. Accuracy is prioritized, though each service cannot
                                be personally verified. After a payment is completed, a confirmation email is sent
                                within 24 hours. This serves as access to the service booked.
                            </p>
                        </li>
                        <li>
                            <p>
                                If you need to cancel, please do so at least 24 hours before your booking time. A small
                                processing fee will be deducted from your refund, which can take anywhere from 10 to 45
                                days to process, depending on your bank. If you cancel too late or don't show up, no
                                refund will be issued.
                            </p>
                        </li>
                        <li>
                            <p>
                                Vehicles are neither owned nor rented out by OneTapDrive. The platform only helps in
                                finding suitable options. Details should be carefully reviewed, questions asked, and
                                decisions made confidently before proceeding. Seeking advice from a professional, such
                                as a legal expert, may also help if any concerns arise
                            </p>
                        </li>
                    </ul>
                    <h3>
                        Limitations
                    </h3>
                    <p>
                        We do our best to keep things running smoothly, but we can't control everything. If something
                        goes wrong—whether it's a loss, delay, or disruption—we're not liable. This includes financial
                        hiccups, data issues, or business interruptions. Even if you've flagged the possibility of a
                        problem, we can't take responsibility for damages.
                    </p>
                    <h3>
                        Updates and Corrections
                    </h3>
                    <p>
                        We work hard to keep our platform accurate and reliable, but mistakes happen. Sometimes, there
                        might be a typo or an outdated piece of information. We fix these as quickly as possible. To
                        improve your experience, we also update our content, features, and services from time to
                        time—often without prior notice.
                    </p>
                    <h3>
                        Payment Terms
                    </h3>
                    <ul>
                        <li>
                            <p>
                                We follow UAE laws since that's where we're based. Payments on OneTapDrive are safe and
                                secure due to SSL encryption and a trusted payment gateway. You can pay using Visa or
                                MasterCard in AED (United Arab Emirates Dirham). Before completing your transaction,
                                it's a good idea to review the terms and policies of your payment provider.
                            </p>
                        </li>
                        <li>
                            <p>
                                If you qualify for a refund, it will be processed back to the same card used for
                                payment. Depending on your bank, this may take up to 45 days.
                            </p>
                        </li>
                        <li>
                            <p>
                                We also follow international regulations, so we can't offer services to anyone involved
                                in activities restricted by organizations like OFAC. This includes individuals or groups
                                linked to terrorism or illegal trade.
                            </p>
                        </li>
                        <li>
                            <p>
                                Lastly, you must be at least 18 years old to use our platform. If you make a payment, be
                                sure to keep your transaction records for your reference. And always keep your account
                                details private.
                            </p>
                        </li>
                    </ul>
                    <h3>
                        Changes to Terms
                    </h3>
                    <p>
                        Sometimes, we update our terms to keep up with changes in the business or to improve the way we
                        work. If anything changes, it'll be official only if it's put in writing by OneTapDrive. It's a
                        good idea to check these terms now and then to stay in the loop.
                    </p>
                    <h3>
                        Platform Availability
                    </h3>
                    <p>
                        The Platform and its Services are maintained to stay accessible as much as possible. However,
                        interruptions may occur, and fault-free operation cannot be ensured. Maintenance or upgrades
                        might lead to temporary unavailability, often without notice, but efforts are made to minimize
                        downtime. No guarantees are provided against service interruptions, and the right to modify,
                        pause, or discontinue any part of the Platform, including user access, is reserved.
                    </p>
                    <h3>
                        Links to Third-Party Websites
                    </h3>
                    <p>
                        Links to external websites are included for convenience but do not serve as endorsements. These
                        sites are managed independently, and responsibility for their content, reliability, or
                        availability lies with their respective administrators. Interactions with such websites are
                        undertaken at your own risk.
                    </p>
                    <h3>
                        Changes to Platform Terms
                    </h3>
                    <p>
                        Terms of use may change at any time without prior notice. Continued use of the Platform confirms
                        acceptance of the latest terms.
                    </p>
                    <h3>
                        Governing Law
                    </h3>
                    <p>
                        All disputes or claims involving the Platform are subject to UAE laws and fall under its
                        jurisdiction.
                    </p>
                    <h3>
                        Airport Transfers
                    </h3>
                    <p>
                        Airport transfers include a single, non-stop journey either starting from or ending at a UAE
                        airport. Additional stops during the trip are not permitted.
                    </p>
                    <h3>
                        Parking Costs
                    </h3>
                    <p>
                        One parking fee is covered for airport transfers and is included in the booking.
                    </p>
                    <h3>
                        Chauffeur Services
                    </h3>
                    <p>
                        The chauffeur service caters to single trips and is not intended for continuous shuttling of
                        multiple passengers. For custom arrangements, inquiries can be sent to info@onetapdrive.com.
                    </p>
                    <h3>
                        Exemption
                    </h3>
                    <p>
                        The platform and all its materials are provided "as is," without any guarantees. We can't
                        promise that everything is always accurate, suitable, or reliable. This includes any implied
                        warranties, such as fitness for a specific purpose, quality, or lack of copyright issues. Use of
                        the Platform outside the UAE must comply with local laws. Exporting or re-exporting materials in
                        violation of UAE laws is strictly prohibited.
                    </p>
                    <h3>
                        For Agents
                    </h3>
                    <p>
                        "We," "our," or "us" refers to OneTapDrive Portal operating under "OneTapDrive," including its
                        team, affiliates, agents, and representatives. The term "platform" covers both the website and
                        app, while "you" or "the client" refers to the user. The Service includes options like managing
                        vehicle listings, banner advertising, highlighting cars, and email marketing. By signing the
                        Advertising Agreement, you agree to the outlined terms. "Month" in all agreements means 30 days.
                    </p>
                    <h3>
                        Payment Terms
                    </h3>
                    <p>
                        Payment must be made in full before services begin. Post-dated checks are accepted if arranged
                        in advance. Pricing and packages are detailed in the Advertising Agreement and may change with
                        notice. Success fees apply as agreed, and VAT may be added based on laws. Payments are processed
                        weekly, and refunds are only issued for valid complaints. If services are not provided, no
                        payments will be made.
                    </p>
                    <h3>
                        Refund Policy
                    </h3>
                    <p>
                        Paid content—whether submitted via credit card, check, or transfer—is non-refundable unless
                        stated otherwise.
                    </p>
                    <h3>
                        Limitation of Liability
                    </h3>
                    <p>
                        No guarantees are provided beyond what is explicitly stated. Liability for issues is limited to
                        the cost of the service paid. External factors like tech issues or delays beyond control are not
                        the platform's responsibility. If claims arise from your content, you must cover any damages or
                        costs.
                    </p>
                    <h3>
                        General Terms
                    </h3>
                    <p>
                        Terms may change and will be updated on the platform. Agreements cannot be transferred without
                        approval. UAE law governs this Agreement, and disputes are resolved in Dubai courts. If any term
                        is invalid, the rest remain effective, with adjustments made to align with original intent.
                    </p>
                    <h3>
                        For Advertisers
                    </h3>
                    <p>
                        "We," "our," or "us" refers to OneTapDrive Portal and its team, while "you" or "the client"
                        refers to users of the Service. The Service includes advertising campaigns, banners, and
                        microsites. By signing the Advertising Agreement, you accept these terms.
                    </p>
                    <h3>
                        Your Obligations
                    </h3>
                    <p>
                        Ensure submitted content complies with laws and isn't misleading or harmful. Using or
                        referencing the platform elsewhere requires prior approval. The platform reserves the right to
                        review or remove content if necessary. Responsibility for data security and communication costs
                        rests with you.
                    </p>
                    <h3>
                        Term and Suspension
                    </h3>
                    <p>
                        The Agreement lasts three months and can be renewed. It may be suspended or terminated for
                        reasons such as non-payment, breach of terms, or submission of false content. If terminated,
                        obligations up to that point remain valid.
                    </p>
                    <h3>
                        Limitation of Liability
                    </h3>
                    <p>
                        The platform is provided "as is" with no guarantees. Liability for damages or tech interruptions
                        is not assumed. Mentioning products or services doesn't imply endorsement.
                    </p>
                    <h3>
                        Governing Law
                    </h3>
                    <p>
                        The Agreement is governed by UAE law, with disputes resolved in Dubai courts. Invalid terms will
                        be replaced to preserve intent without affecting the rest of the Agreement.
                    </p>
                    <p>
                        By focusing on simplicity and clarity, these terms aim to make your experience with OneTapDrive
                        stress-free and transparent. Happy renting!
                    </p>
                </div>
            </div>
        </section>
    )
}

export default page