...
function VendorLogin() {
    ...
    const loginMutation = useMutation({
        mutationFn: login,
        onSuccess: (res) => {
            if (res?.data?.user_details?.status) {
    ...
    return (
        <>
            ...
            {approved && (
                <div class="vendorLoginPopup">
                    <div class="popupContent">
                        <h2>Welcome to <span>OneTapDrive</span></h2>
                        <p>You haven{"'"}t been approved by the admin yet. Please wait for confirmation.</p>
                        <Link href={"/"} className="themeBtn">Explore Website</Link>
                    </div>
                </div>
            )}
        </>
